...
const ProjectCard = ({
  ...
  return (
    <div className="mb-8 max-w-[1180px]">
      <div className="h-52 max-h-52 md:max-h-72 w-full md:min-h-72 md:h-72 rounded-t-xl relative group">
        <Image
        ...
        <div className="overlay items-center justify-center absolute top-0 left-0 w-full h-full bg-[#181818] bg-opacity-0 hidden group-hover:flex group-hover:bg-opacity-80 transition-all duration-1000">
          ...
          {(tag.includes("Mobile") && tag.includes("Download")) ? (
            <button
              ...
              className="h-14 w-14 border-2 relative rounded-full border-[#ADB7BE] hover:border-white group/link"
            >
              <ArrowDownTrayIcon className="w-8 h-8 text-[#ADB7BE] absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 cursor-pointer group-hover/link:text-white" />
            </button>
          ) : null}
        </div>
      </div>
